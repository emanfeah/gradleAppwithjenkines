pipeline {
    ...
    stages {
        ...
        stage('Package') {
            ...
            post{
                success{
                    archiveArtifacts artifacts: 'build/libs/emanfeah-0.0.1-SNAPSHOT-plain.jar', followSymlinks: false
                }
            }
        }
        stage('Publish artifacts to S3 Bucket') {
            steps {
                ...
                sh "aws s3 cp ./build/libs/emanfeah-0.0.1-SNAPSHOT-plain.jar s3://$AWS_S3_BUCKET/$ARTIFACT_NAME"
            }
         }
        stage('Deploy') {
            steps {
